...
module.exports = {
  ...
  // background:
  //   'frost'    带色磨砂（默认）
  //              近黑墨底压到 86% 不透明，四角各透出一团深饱和彩晕，
  //              叠 64px 细网格和一层实测标定过的胶片颗粒。导航左对齐、
  //              序号在标签正上方，打开时下划线从左抹开。
  //   'ink'      墨玻璃 + 通栏索引表（上一版默认，参数原样保留）
  ...
  //
  // ⚠️ 'frost' 的含义变了。上一版的 frost 是「半透明蓝 + 背景模糊」，那套
  //    效果现在用 'gradient' 就能近似；这个名字让给了新的带色磨砂。
  //
  // 为什么不用浅色底：上游在菜单打开时把 logo 反白（纯白 SVG），浅底会让
  // logo 直接消失。修它要 `:has()`，代价比收益大，所以磨砂做在深底上。
  menu: {
    background: 'frost',
    // ---- frost / ink 用这两个 ----
