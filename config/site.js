...
module.exports = {
  ...
  // background:
  //   'ink'      墨玻璃 + 通栏索引表（默认）
  //              近黑墨底 + 背景模糊，导航改成每条一行、上边发丝线、
  //              右侧 01/02/03/04 序号，打开时逐条从下方抬起。
  //   'aurora'   深蓝渐变 + 三团极光光斑 + 细噪点
  //   'gradient' 纯线性渐变，最省电，动画和噪点都不要
  //   'frost'    半透明深色 + 背景模糊，3D 场景隐约透出来
  //   'none'     什么都不注入，保持上游那个「没有背景」的状态
  //
  // 为什么默认换成 ink：aurora 那套深蓝和加载页曾经用的是完全相同的三个色、
  // 相同的手法，同一个站里两处大面积色块长得一模一样。现在两边刻意分家。
  menu: {
    background: 'ink',
    // ---- ink 用这两个 ----
    // 墨色。对白字的对比度必须 ≥ 7:1，不够构建会直接报错（不是告警）。
    ink: '#0c0c0e',
    // 右侧那列 01/02/03/04 序号。用 CSS 计数器生成，条目数变了会自己跟着变。
    index: true,
    ...
    // ---- 共用 ----
    noise: true,  // 细噪点，用来打散深色渐变上的色带（ink / frost 不需要）
    motion: true, // ink 是逐条抬起，aurora 是极光飘动；
                  // 系统开了「减少动态效果」时都会自动停
  },
